...
void HdmiRx_Init(void)
{
	...
	//add by wzl
	HdmiRx_Set(0x66, 0x03, 0x01);//Video stable condition selector 01: clock stable
	HdmiRx_Set(0x71, 0xe0, 0xe0);
	HdmiRx_Set(0x71, 0x10, 0x10);//The tolerance value for video stable

	//HdmiRx_Set(0x22, 0x10, 0x10);//Reg_AutoEQ: RS updated with AutoEQ
	HdmiRx_Set(0x22, 0x10, 0x00);//RS hold
	HdmiRx_Set(0x26, 0x20, 0x20);//Manually set RS value
	HdmiRx_Set(0x27, 0x80, 0x80);//all 3 channels use RS value of 0x27 whe

	HdmiRx_Set(0x27, 0x7f, 0x7f);//RS value of CH0
	HdmiRx_Set(0x28, 0x7f, 0x7f);//RS value of CH1
	HdmiRx_Set(0x29, 0x7f, 0x7f);//RS value of CH2
...
void HdmiRx_Irq(void)
{
	...
	if(u8Reg05[0] ||u8Reg05[1] ||u8Reg05[2] ||u8Reg05[3] ||u8Reg05[4] ){
		iTE_HRx_Msg(("\n\rHdmiRx_Irq S 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X\n\r", u8Reg05[0], u8Reg05[1],u8Reg05[2], u8Reg05[3], u8Reg05[4]));
		if(u8Reg05[0]) {
			if(u8Reg05[0] & 0x01){
				if(u8Reg05[5] & 0x01){
...
void HdmiRx_Sw_Timer_IRQ(void)
{
	...
	iTE_MEQ_Msg(("HdmiRx_Sw_Timer_IRQ\n\r"));
...
void HdmiRx_GetTmdsClk(void)
{
	...
	iTE_MEQ_Msg(("TMDSCLK = %d.%03dMHz\n\r", (int)(u32TmdsClk/1000), (int)(u32TmdsClk%1000)));
...
void HdmiRx_GetVideoTiming(VTiming *pCurVTiming)
{
	...
	pCurVTiming->PCLK			= stIt6662->u32RefClk * 255 / HdmiRx_R(0x9A);
...
//	iTE_HRx_Msg(("TMDSCLK = %d.%03dMHz\n\r", u32TmdsClk/1000, u32TmdsClk%1000));
	iTE_HRx_Msg(("PCLK = %d.%03dMHz\n\r", (int)(pCurVTiming->PCLK / 1000), (int)(pCurVTiming->PCLK % 1000)));
	iTE_HRx_Msg(("FrameRate = %dHz\n\r", (int)(pCurVTiming->PCLK * 1000 / pCurVTiming->HTotal / pCurVTiming->VTotal)));
	iTE_HRx_Msg(("HActive = %d\n\r", pCurVTiming->HActive));
	iTE_HRx_Msg(("VActive = %d\n\r", pCurVTiming->VActive));
	iTE_HRx_Msg(("HTotal = %d\n\r", pCurVTiming->HTotal));
	iTE_HRx_Msg(("VTotal = %d\n\r", pCurVTiming->VTotal));

	iTE_HRx_Msg(("HFrontPorch = %d\n\r", pCurVTiming->HFrontPorch));
	iTE_HRx_Msg(("HSyncWidth = %d\n\r", pCurVTiming->HSyncWidth));
	iTE_HRx_Msg(("HBackPorch = %d\n\r", pCurVTiming->HBackPorch));
	iTE_HRx_Msg(("VFrontPorch = %d\n\r", pCurVTiming->VFrontPorch));
	iTE_HRx_Msg(("VSyncWidth = %d\n\r", pCurVTiming->VSyncWidth));
	iTE_HRx_Msg(("VBackPorch = %d\n\r", pCurVTiming->VBackPorch));
	iTE_HRx_Msg(("Scan Mode = %s\n\r", (pCurVTiming->ScanMode) ? "Progressive":"InterLaced"));
	iTE_HRx_Msg(("HSync Pol = %s\n\r", (pCurVTiming->HPolarity) ? "Positive":"Negative"));
	iTE_HRx_Msg(("VSync Pol = %s\n\r", (pCurVTiming->VPolarity) ? "Positive":"Negative"));
}
//****************************************************************************
...
//	iTE_HRx_Msg(("Output Color Depth = %d bits per pixel\n\r", OutCD*6 + 24));
	...
	if(u8Temp & 0x40) {				// MHL_Mode
		...
		iTE_HRx_Msg(("BUS MODE: MHL %s Mode (%s)\n\r", (u8Temp == 0x02)?"PackPixel":"24 bits", (u8Temp & 0x02)?"HDMI":"DVI"));

	} else {							// HDMI Mode
		iTE_HRx_Msg(("BUS MODE:  %s Mode\n\r", (((u8Temp & 0x02) && (!(HdmiRx_R(0x51) & 0x01))) || ((HdmiRx_R(0x0B) & 0x02) && (HdmiRx_R(0x51) & 0x01)))?"HDMI" : "DVI"));
	}
...
void HdmiRx_CscOutputSet(iTE_u8 u8OutClr)
{
	...
	printf("g_u8InClr = %d, u8OutClr = %d\n\r", g_u8InClr, u8OutClr);
	if(g_u8InClr && (u8OutClr == 0)){		// YUV to RGB
		if(g_u8InClr == 1){
			//g_u8InClr = 0x01 u8OutClr = 0x00
			iTE_Msg(("YUV422 to RGB444\n\r"));
			Csc_Yuv422ToRgb();
		}else{
			//g_u8InClr = 0x02 u8OutClr = 0x00
			iTE_Msg(("YUV444 to RGB444\n\r"));
			Csc_Yuv444ToRgb();
		}
	}else if((g_u8InClr == 0) && u8OutClr){	//RGV to YUV
		if(u8OutClr == 1){
			//g_u8InClr = 0x00 u8OutClr = 0x01
			iTE_Msg(("RGB444 to YUV422\n\r"));
			Csc_RgbToYuv422();
		}else{
			//g_u8InClr = 0x00 u8OutClr = 0x02
			iTE_Msg(("RGB444 to YUV444\n\r"));
			Csc_RgbToYuv444();
		}
	}else{								// Bypass CSC
		//0x65[5:4] Output color space 
		//00: RGB444 
		//01: YUV422 
		//10: YUV444
		...
		iTE_Msg(("CSC bypass\n\r"));
		if(u8OutClr == 1){
			//g_u8InClr = 0x01 u8OutClr = 0x01
			HdmiRx_Set(0x65, 0x33, 0x10);
		}
		else if(u8OutClr == 2){
			HdmiRx_Set(0x65, 0x33, 0x20);
		}else{
			//g_u8InClr = 0x00 u8OutClr = 0x00
			//fix by wzl
			//HdmiRx_Set(0x65, 0x33, 0x20);
			HdmiRx_Set(0x65, 0x33, 0x00);
		}
	}
...
void HdmiRx_HpdOut(iTE_u8 u8OutStatus)
{
	...
	{
		iTE_HRx_Msg(("* * * * * * * * * * * * * * * * * * * * * * * * * *\n\r"));
		iTE_HRx_Msg(("* * * * * * *HdmiRx_HpdOut IT6662_HPD_L * * * * * *\n\r"));
		iTE_HRx_Msg(("* * * * * * * * * * * * * * * * * * * * * * * * * *\n\r"));
		//hpd
		HdmiRx_Set(0xB0, 0x03, 0x01);
		//cbus
		HdmiRx_Set(0xB0, 0x0C, 0x04);

		//HPD pull down signal
		HdmiRx_Set(0xBA, 0x20, 0x20);

		//CBUS pull down signal
		HdmiRx_Set(0xBB, 0x20, 0x20);
	}
	else
	{
		iTE_HRx_Msg(("* * * * * * * * * * * * * * * * * * * * * * * * * *\n\r"));
		iTE_HRx_Msg(("* * * * * * *HdmiRx_HpdOut IT6662_HPD_H * * * * * *\n\r"));
		iTE_HRx_Msg(("* * * * * * * * * * * * * * * * * * * * * * * * * *\n\r"));
		//hpd
		HdmiRx_Set(0xB0, 0x03, 0x03);
		//cbus
		HdmiRx_Set(0xB0, 0x0C, 0x0C);

		//HPD pull up signal
		HdmiRx_Set(0xBA, 0x20, 0x00);

		//CBUS pull up signal
		HdmiRx_Set(0xBB, 0x20, 0x00);
	}
	HdmiRx_Bank(0);
	iTE_HRx_Msg(("\n\r"));
#endif
...
void HdmiRx_TrgHpd(iTE_u8 u8TimeCnt)
{
	...
	iTE_Msg(("HdmiRx_TrgHpd =%x, %02X, %02X\n\r", u8TimeCnt, u8Temp, stIt6662->u8HpdTimerCnt));
...
//		iTE_HTx_Msg(("[%d] AV.H:0x%08X \n\r", u8Temp,  u32Hash[u8Temp]));
	...
	if( HdmiRx_R(0x31) & 0x40){
		iTE_HTx_Msg(("\n\r\n\r***KSV Set Ready Success, DevCnt = %d**\n\r", stIt6662->u16DevCnt) );
	}else {
		iTE_HTx_Msg(("*************************WARING: KSV Set Ready Fail, DevCnt = %d****************************************\n\r", stIt6662->u16DevCnt) );
	}
#if 0
	iTE_HTx_Msg(("M0: 0x%02X%02X%02X%02X%02X%02X%02X%02X\n\r", u8M0[7], u8M0[6], u8M0[5], u8M0[4], u8M0[3], u8M0[2], u8M0[1], u8M0[0]));

	iTE_HTx_Msg(("HDMI Rx BV.H : 0x%08X	0x%08X	0x%08X	0x%08X	0x%08X\n\r", u32Hash[0], u32Hash[1], u32Hash[2], u32Hash[3], u32Hash[4]));
	...
	if(g_u8CurDev == IT6662_ROOT){
		...
		if(HdmiRx_R(0x0C) & 0x40){
			...
			if(u8Temp & 0x40){	// MHL Mode
				...
				HdmiRx_Set(0xB8, 0x80, 0x01);//0x80);
...
{
	iTE_u8	u8Reg1D5[3];
	iTE_u8	u8Cnt1, u8Cnt2, u8Temp1, u8Temp2 = 0;

	u8Temp1 = HdmiRx_R(0x26);
	if(u8Temp1 & 0x20 ){
		return;
	}
	iTE_MEQ_Msg(("******** HdmiRx_AutoEqFill ********\n\r"));
...
//	HdmiRx_Set(0x26, 0x20, 0x20);
	...
	if(HdmiRx_R(0x0A)&0x40){
		...
		if(u8Reg1D5[0] < MinEQValue){ u8Reg1D5[0] = MinEQValue;}
		if(u8Reg1D5[1] < MinEQValue){ u8Reg1D5[1] = MinEQValue;}
		if(u8Reg1D5[2] < MinEQValue){ u8Reg1D5[2] = MinEQValue;}
	}
	HdmiRx_W(0x22, 0x00);
	...
	//				iTE_MEQ_Msg((" u8Cnt2 = %d\n\r", u8Cnt2));
		...
		u8Temp2 /= 3;
		iTE_MEQ_Msg(("AutoEQ Value u8Temp2 = %d, %X, %X, %X\n\r", u8Temp2, u8EqValue[u8Temp2][0], u8EqValue[u8Temp2][1], u8EqValue[u8Temp2][2]));
		//HdmiRx_Wb(0x27, 0x03, u8Reg1D5);
...
void HdmiRx_EccChk(void)
{
	...
	iTE_MEQ_Msg(("Reg0x27=0x%02X, 0x%02X, 0x%02X\n\r", HdmiRx_R(0x27), HdmiRx_R(0x28), HdmiRx_R(0x29)));
}
//****************************************************************************
void HdmiRx_ManulEqTrigger(void)
{
	...
	if(g_bManulEqEn == 0){
		iTE_u8	u8Temp;
		g_u8ManulEqCnt = 0;
		for(u8Temp = 0; u8Temp < ManulEqStage; u8Temp++){
			g_u16EqErrCnt[u8Temp] = 0xFFFF;
		}
		g_bManulEqEn = 1;
		HdmiRx_ManulEqSet();
...
void HdmiRx_ManulEqSet(void)
{
	...
	if(g_bManulEqEn && (g_u8EqChkCnt == 0)){
		...
		if(g_u8ManulEqCnt < ManulEqStage){
			...
			iTE_MEQ_Msg(("Manul EQ Value: %02X, %02X, %02X, %02X\n\r", u8EqValue[u8ManulEqTab[g_u8ManulEqIdx]][0], u8EqValue[u8ManulEqTab[g_u8ManulEqIdx]][1], u8EqValue[u8ManulEqTab[g_u8ManulEqIdx]][2], u8EqValue[u8ManulEqTab[g_u8ManulEqIdx]][3]));
//			HdmiRx_Set(0x20, 0x3F, u8EqValue[u8ManulEqTab[g_u8ManulEqIdx]][3]);
			...
			iTE_MEQ_Msg(("\n\r ErrList = 0x%02X\n\r", u8EqErrList));
			u8BestPtr = 0;
			iTE_MEQ_Msg(("\n\r*****u16EqErrCnt[0] = %X****\n\r\n\r", g_u16EqErrCnt[u8ManulEqTab[0]]));
			for(u8Temp = 1; u8Temp < ManulEqStage; u8Temp++){
				iTE_MEQ_Msg(("\n\r*****u16EqErrCnt[%d] = %X****\n\r\n\r", u8Temp, g_u16EqErrCnt[u8ManulEqTab[u8Temp]]));
				if(g_u16EqErrCnt[u8ManulEqTab[u8BestPtr]] > g_u16EqErrCnt[u8ManulEqTab[u8Temp]]){
					u8BestPtr = u8Temp;
				}
			}
			//HdmiRx_Wb(0x27, 0x03, u8ManulEQ[u8BestPtr]);
			//iTE_MEQ_Msg(("\n\r*****BestPtr=%d, ManulEqValue: %02X, %02X, %02X****\n\r\n\r", u8BestPtr, u8ManulEQ[u8BestPtr][0], u8ManulEQ[u8BestPtr][1], u8ManulEQ[u8BestPtr][2]));
//			HdmiRx_Set(0x20, 0x3F, u8EqValue[u8ManulEqTab[u8BestPtr]][3]);
			HdmiRx_Wb(0x27, 0x03, u8EqValue[u8ManulEqTab[u8BestPtr]]);
			iTE_MEQ_Msg(("Manul EQ Value: %02X, %02X, %02X, %02X\n\r", u8EqValue[u8ManulEqTab[u8BestPtr]][0], u8EqValue[u8ManulEqTab[u8BestPtr]][1], u8EqValue[u8ManulEqTab[u8BestPtr]][2], u8EqValue[u8ManulEqTab[u8BestPtr]][3]));
			g_bManulEqEn = 0;
			g_bManulEqEnDone = 1;
			g_u8EqReChkCnt++;
