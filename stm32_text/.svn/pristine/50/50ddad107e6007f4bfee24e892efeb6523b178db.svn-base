...
iTE_u8 g_u8RdStart = 0;
iTE_u8 g_u8RdCnt = 0;
iTE_u8 g_u8EdidBlk = 0;
...
//				iTE_HTx_Msg(("********************************************ERROR:************************\n\r"));
		...
		if(u8Reg08 & 0x40) {
			...
			if(u8Tmp){
				...
				iTE_HTx_Msg(("Reset HDMI Tx Packet FIFO ...\n\r"));
			}
		}
#if 0
		if(((HdmiTx_R(0x48) & 0x03) != 0x03) && ((HdmiTx_R(0x88) & 0x03) != 0x03)){
...
//	u16Temp /= (1<<u8PreDivSel);
	...
	u32PClk <<= u8PreDivSel;
	iTE_HTx_Msg(("TxPCLK = %d.%dMHz \n\r", (int)(u32PClk/1000), (int)(u32PClk%1000)));
	return u32PClk;
...
iTE_HTx_Msg(("\n\rHdmiTx_PortIRQ: %d  0x%02X, 0x%02X S\n\r",g_u8TxSel, u8Reg40[0], u8Reg40[1]));
	if(u8Reg40[0] & 0x01) {
		if(u8Reg48 & 0x01){
...
//				HdmiTx_PortEnable();
				...
				iTE_HTx_Msg(("%s: HPD L2H\n\r", __func__));
				HdmiTx_PortEnable(u8Reg48 & 0x02);

				stIt6662->u8EdidCheck |= g_u8TxShift;
			}
#if (USING_HDCP_CTS == iTE_TRUE)	// HDCP CTS
				stIt6662->u8EdidCheck |= g_u8TxShift;
#endif
		}else{
...
//			if(stIt6662->u8HdmiTxDoHdcp & g_u8TxShift){
			if(1){
				if(stIt6662->u8TxPortEn & g_u8TxShift){				//should be removed
					...
					iTE_HTx_Msg(("Enable HDCP Fire %d\n\r", stIt6662->u8HDCPFireCnt[g_u8TxSel]));
	...
	if(u8Reg40[1] & 0x80){
		...
		iTE_HTx_Msg(("HDCP Sync Detect Fail, %d\n\r", stIt6662->u8TxSyncDetCnt[g_u8TxSel] ));
		HdmiTx_HdcpStatus();
	...
	iTE_HTx_Msg(("HdmiTx_PortIRQ: %d E\n\r\n\r",g_u8TxSel));
}
}
//****************************************************************************
iTE_u8 HdmiTx_Sw_Timer_IRQ(iTE_u8 u8TxSel)
{
	...
	if(stIt6662->u8TxRptRdyCnt[u8TxSel] ){
		...
		iTE_HTx_Msg(("%s: u8TxSel: %d RptRdyCnt = %d\n\r", __func__, u8TxSel, stIt6662->u8TxRptRdyCnt[u8TxSel]));
	}
...
#endif
	iTE_HTx_Msg(("HdmiTx_PortDisable %d\n\r", g_u8TxSel));
}
//****************************************************************************
...
//	iTE_u8 	u8HdmiTxSel = g_u8TxSel;
	...
	iTE_HTx_Msg(("\n\r HdmiTx_PostSpenHdcp S %d\n\r", g_u8TxSel));
	BStatus = u8Buf;
	M0 = &u8Buf[2];

//	iTE_HTx_Msg(("HDCP Interrupt Status = 0x%02X\n\r", HdmiTx_R(0x41 + g_u8TxOffset)));
	HdmiTx_DdcReset();
...
//		iTE_HTx_Msg(("M0: 0x%02X%02X%02X%02X%02X%02X%02X%02X\n\r", M0[7], M0[6], M0[5], M0[4], M0[3], M0[2], M0[1], M0[0]));
		...
		if((BStatus[0] & 0x80) || (BStatus[1] & 0x08)) {
			...
			iTE_HTx_Msg(("ERROR: Max. Device or Cascade Exceeded\n\r"));
			return 0;
		} else {
			iTE_u32 	u32Hash[5];
...
for(u8Temp =0; u8Temp < stIt6662->u16DevCnt; u8Temp++){
	iTE_HRx_Msg((" KSV[%d] = %02X%02X%02X%02X%02X\n\r", u8Temp, pu8KsvList[u8Temp*5], pu8KsvList[u8Temp*5+1], pu8KsvList[u8Temp*5+2], pu8KsvList[u8Temp*5+3], pu8KsvList[u8Temp*5+4]));
}
#endif
...
//			HdmiRx_W(0x2D, u8DevCnt);
			...
			for(u8Temp = 0; u8Temp < 5; u8Temp++){
				...
				if(((u32Hash[u8Temp] & 0xFF) != u8BV[0])
					...
					u1ShaPass = iTE_FALSE;
					iTE_HTx_Msg(("[%d] BV.H: 0x%02X%02X%02X%02X,  AV.H:0x%08X \n\r", u8Temp, u8BV[3], u8BV[2], u8BV[1], u8BV[0], u32Hash[u8Temp]));
					break;
			...
			for(u8Temp = 0; u8Temp < 20; ){
				...
				if((pu8Hash[u8Temp++]  != u8BV[0])
					...
					u1ShaPass = iTE_FALSE;
					iTE_HTx_Msg(("[%d] BV.H: 0x%02X%02X%02X%02X,  AV.H:0x%08X \n\r", u8Temp>>2, u8BV[3], u8BV[2], u8BV[1], u8BV[0], u32Hash[u8Temp]));
					break;
		...
		HdmiTx_Bank(0);
		HdmiTx_SaveKvsHdcp();
		iTE_HTx_Msg(("\n\r HdmiTx_PostSpenHdcp E %d\n\r", g_u8TxSel));
#if (USING_HDCP_RPT_RDY_CHK == iTE_TRUE)
	}else{
		iTE_HTx_Msg(("[%d]u8TxRptRdyCnt=%d\n\r", g_u8TxSel, stIt6662->u8TxRptRdyCnt[g_u8TxSel]));
...
#if 0
{
	...
	HdmiRx_Bank(0);
	for(u8Temp=0; u8Temp < stIt6662->u16DevCnt; u8Temp++){
		iTE_HTx_Msg(("KSV List [%d] = 0x %02X %02X %02X %02X %02X\n\r", u8Temp, u8KsvList[u8Temp*5+4],u8KsvList[u8Temp*5+3],u8KsvList[u8Temp*5+2],u8KsvList[u8Temp*5+1],u8KsvList[u8Temp*5]));
	}
}
#endif
...
//		for(u8Temp = 0; u8Temp < stIt6662->u16DevCnt; u8Temp++){
		for(u8Temp = 0; u8Temp < u8DevCnt; u8Temp++){
			...
			iTE_HTx_Msg(("KSV List [%d] = 0x %02X %02X %02X %02X %02X\n\r", u8Temp, u8KsvList[4],u8KsvList[3],u8KsvList[2],u8KsvList[1],u8KsvList[0]));
		}
		HdmiRx_Bank(0);
	}
	}
...
//	stIt6662->u8HdmiTxDoHdcp = HdmiTx_R(0x19) & 0x0F;
	stIt6662->u8TxDoHdcp = stIt6662->u8TxPortEn;
#if 1
	stIt6662->u8TxSpenHdcp = stIt6662->u8TxDoHdcp;
#else
...
//			HdmiRx_SpenHdcp();
	...
	HdmiTx_W(0x23, 0x0F);				// Enable all Tx AVMute
	iTE_HTx_Msg(("\n\rHdmiTx_AuthHdcp E\n\r"));
}
// 20140826_E
//****************************************************************************
...
//	iTE_u8	u8OutClr;
	...
	for(u8Temp = 0; u8Temp < 4; u8Temp++)
	{
		...
		if(stIt6662->u8TxHpdStatus & u8TmpShift)
		{
			...
			if(u8Temp == 0)
			{
				u8IsHdmiMode = (HdmiRx_R(0x0A) & 0x02) && g_stEdidInfo[u8Temp].pstVsdbInfo.u8PaAdr;
			}
			else
			{
				u8IsHdmiMode = g_stEdidInfo[u8Temp].pstVsdbInfo.u8PaAdr;
			}
			if(u8IsHdmiMode)
			{
				if((HdmiTx_R(0x22) & u8TmpShift) == 0)
				{
					iTE_HTx_Msg(("%X change TX output mode\n\r", u8Temp));
					HdmiTx_Set(0x04, u8TmpShift, u8TmpShift);					//Software VCLK domain reset
...
iTE_u1 HdmiTx_HdcpRead(iTE_u8 u8HdcpOffset, iTE_u16 u16ByteNum)
{
	...
	if(HdmiTx_DdcWait() == iTE_FALSE) {
		iTE_HTx_Msg(("ERROR: TX DDC HDCP Read Fail !!!\n\r"));
		bStatus = iTE_FALSE;
	}
	HdmiTx_Set(0x4A + u8Offset, 0x01, 0x00);					// Disable PC DDC Mode
	return bStatus;
...
#define DDCWAITNUM      (50)
	...
	}while(((HdmiTx_R(0x50 + u8Offset) & 0x80) == 0x00) && (++u8DdcWaitCnt < DDCWAITNUM));
#else
	while(((HdmiTx_R(0x50 + u8Offset) & 0x80) == 0x00) && (u8DdcWaitCnt++ < DDCWAITNUM)){
		iTE_Sleep_ms(DDCWAITTIME);
	};
	...
	if(u8DdcWaitCnt >= DDCWAITNUM) {
		iTE_HTx_Msg(("DDC Bus Status = 0x%02X\n\r", HdmiTx_R(0x50 + u8Offset) & 0xFE));
		iTE_HTx_Msg(("ERROR: DDC Bus Wait Time Out:  \n\r"));
		iTE_HTx_Msg(("u8Bank=0x%02X, u8Offset=0x%02X\n\r",g_u8TxBank, g_u8TxOffset));
...
#define DDC_RETRY_CNT (15)
	...
	if(g_u8TxSel == HDMI_TX_A)
	{
		g_u8RdStart = u8RdStart;
		g_u8RdCnt = u8RdCnt;
		g_u8EdidBlk = u8EdidBlk / 2;
		return EDID_NO_ERR;
	}
	...
	do{
		...
		if(iTE_FALSE == HdmiTx_DdcWait()){
			u8Temp++;
			iTE_Edid_Msg(("*****ERROR*****: DDC EDID Read Fail, Read Start: 0x%02X [%d/%d]\n\r", u8RdStart, u8Temp, DDC_RETRY_CNT));
			iTE_Sleep_ms(100);
			if(u8Temp >= DDC_RETRY_CNT){
	...
	}while(u8Temp < DDC_RETRY_CNT);

	return EDID_READ_ERR;
}
//****************************************************************************
iTE_u8 _CODE u8Edid6801_M6[256] = {
...
void HdmiTx_EdidRead(iTE_u8	u8RdCnt, iTE_pu8 pu8RdEdid)
{
	if(g_u8TxSel == HDMI_TX_A)
	{
		//printf("g_u8RdStart: %d g_u8RdCnt: %d g_u8EdidBlk: %d\n\r", g_u8RdStart, g_u8RdCnt, g_u8EdidBlk);
		if(g_u8EdidBlk == 0)
		{
			memcpy(pu8RdEdid, u8Edid6801_M6 + g_u8RdStart, u8RdCnt);
		}
		else if(g_u8EdidBlk == 1)
		{
			memcpy(pu8RdEdid, u8Edid6801_M6 + g_u8RdStart + 128, u8RdCnt);
		}
		return;
	}
	//HdmiTx_Rb(0x51 + g_u8TxOffset, u8RdCnt, pu8RdEdid);
	My_HdmiTx_Rb(0x51 + g_u8TxOffset, u8RdCnt, pu8RdEdid);
}
//****************************************************************************
