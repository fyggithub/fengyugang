...
void CecRx_OtpFeature(void)
{
	switch(g_stCecCur->stCecRxCmd.ucOpcode){
		...
		case	eActiveSource:
				...
				iTE_Cec_Msg(("[%02X%02X]\n\r", g_stCecCur->stCecRxCmd.ucOperand1, g_stCecCur->stCecRxCmd.ucOperand2));
...
void CecRx_SysInfoFeature(void)
{
	switch(g_stCecCur->stCecRxCmd.ucOpcode){
		case	eCECVersioin:				//<MS> <0x9E> <CEC version>
				iTE_Cec_Msg(("\t CEC Version =%bX!!!\n\r",g_stCecCur->stCecRxCmd.ucOperand1));
			break;
		...
		case	eReportPhysicalAddress:	// <DB> <0x84> <Physicall Adress> <Device type>, example:[0x5F 0x84 AB CD 0]
				iTE_Cec_Msg((" PhyAdr = %02X%02X, DEV_TYP = %d \n\r", g_stCecCur->stCecRxCmd.ucOperand1, g_stCecCur->stCecRxCmd.ucOperand2, g_stCecCur->stCecRxCmd.ucOperand3));
				// ToDo
			break;
		...
		case eVendorCommandWithID :	//Vendor Command With ID			// Vendor Specific
				if(u8CmdSize > 16){
					...
					for(u8Temp=5; u8Temp<u8CmdSize; u8Temp++){
						iTE_Cec_Msg(("%02X\n\r",*((&g_stCecCur->stCecRxCmd)+u8Temp)));
					}
					// ToDo
				}
			break;
