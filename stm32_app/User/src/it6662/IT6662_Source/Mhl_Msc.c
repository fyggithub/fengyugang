...
#include "Mhl_Msc.h"
#include "..\IT6662_Customer\Mhl_RcpUcpHandler.h"
#include "..\IT6662_Customer\IT6662_DefaultEdid.h"
#include "Edid.h"
...
void Msc_ParseDevCap(iTE_u8 ucOffset, iTE_u8 ucData)
{
	switch(ucOffset){
		...
		case	2:
				...
				iTE_MRx_Msg(("POW=%d, PLIM=%d \n\r", (int)((ucData&0x10)>>4), (int)((ucData&0x60)>>5)));
			break;
		case	3:
...
MSC_STATUS Msc_Fire(MSC_PKT_ID eMscCtlPkt, MSC_CMD *MscCmd)
{
	...
	switch(eMscCtlPkt){
		...
		case	MSC_PKT_WRITE_BURST:
				...
				iTE_MRx_Msg(("MSC_FIRE  Reg0x%x=0x%x\n\r",(int)MscFire[eMscCtlPkt][0], (int)MscFire[eMscCtlPkt][1]));
			break;
#if 0
...
void Msc_RcpKeyParse( iTE_u8 ucRcpKey )
{
	if(pucRcpString[ucRcpKey&0x7f] != NULL){
     		iTE_MRx_Msg(("KeyCode=0x%02X ==> [%s]\n\r ", (int)ucRcpKey, pucRcpString[ucRcpKey&0x7f]));
	}else{
		...
		case	MHL3D_REQ_START:
				...
				if(st3DParse.uc3DInfor){
					...
					*e3DReqState = MHL3D_REQ_WRT;
					...
					iTE_MRx_Msg(("*************ERROR: st3DParse.uc3DInfor mem alloc fail  ***************\n\r" ));
