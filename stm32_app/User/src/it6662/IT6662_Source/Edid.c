...
EDID_STAT Edid_ExtSaveBlock(iTE_u8 u8EdidBlk)
{
	...
	while(u8RdStart < u8RdEnd){
		...
		for(i = 0; i < u8RdCnt; i++)
		{
			if((i % 16 == 0) && (i != 0))
				iTE_Edid_Msg(("\n\r"));
			iTE_Edid_Msg(("0x%02x ", u8EdidBuf[i]));
		}
		iTE_Edid_Msg(("\n\r"));
#endif	
#if 0
		if(u8EdidBlk == 0){
			for(u8Temp = 0; (u8RdStart+u8Temp) < 8; u8Temp++){
				if(u8RdEdid[u8Temp] != u8EdidBlk0Header[u8RdStart+u8Temp]){
					iTE_Edid_Msg(("*****ERROR*****: EDID Block 0 Header Check Fail, EDID_0[0x%02X] = 0x%02X\n\r", u8RdStart+u8Temp, u8EdidBuf[u8Temp]));
					return EDID_HEADER_ERR;
				}
			}
		}
...
void Edid_ExtCheck(iTE_u8 u8TxSel)
{
	...
	if(stIt6662->u8EdidCheck & (0x01 << u8TxSel))
	{
		...
		if(stIt6662->u8TxHpdStatus & g_u8TxShift)
		{
			...
			HdmiTx_Bank(0);
			if(g_stEdidInfo[u8TxSel].u8ExtBlkCnt == 0)
			{
				eStatus = EDID_CEA_ERR;
			}
...
EDID_STAT Edid_ExtParse(EdidInfo *pstEdidInfo)
{
	...
	if(eEdidRdSta == EDID_NO_ERR)
	{
		for(u8Temp = 0; u8Temp < pstEdidInfo->u8ExtBlkCnt; )
		{
			...
			if(eEdidRdSta == EDID_NO_ERR)
			{
				//Edid_InfoShow(pstEdidInfo);
			}
		}
	}

	if(pstEdidInfo->pstVsdbInfo.u8PaAdr)
	{
		return EDID_NO_ERR;
	}
	else
...
//					Edid_InfoShow(pstEdidInfo);
	...
	u8Temp = pstEdidInfo->pstVsdbInfo.u8PaAdr;
	if(u8Temp){
		...
		u8ChkSum = Edid_R(0xFF) + u8TmpPa[0] + u8TmpPa[1] - DefaultEdid_PA_AB - DefaultEdid_PA_CD;
...
EDID_STAT Edid_ExtBlk0Parse(EdidInfo *pstEdidInfo)
{
	...
	Edid_DtdParse(0x36, pstEdidInfo, 0, NULL);
...
EDID_STAT Edid_ExtCeaParse(iTE_u8 u8EdidBlk, EdidInfo *pstEdidInfo, iTE_u1 bPDefault3DVic)
{
	...
	{
		//Blk1 Dtd
		Edid_DtdParse(u8RdEnd, pstEdidInfo, u8EdidBlk, NULL);
		Edid_Dtd2Vic(pstEdidInfo);
		return EDID_NO_ERR;
	}
	else
...
iTE_u8 Edid_ColorDepthCheck(void)
{
	...
	for(u8Temp=0; u8Temp<4; u8Temp++){
		if(stIt6662->u8TxHpdStatus & (1 << u8Temp)){
			u8TmpB6 = u8TmpB6 & ((g_stEdidInfo[u8Temp].pstVsdbInfo.u8B6 & 0x38) | 0x87);	//un-support DC_48 bits
		}
	}
...
void Edid_InfoShow(EdidInfo *pstEdidInfo)
{
	...
	iTE_Edid_Msg(("PA_Adr = 0x%02X, PA=0x%02X%02X\n\r", pstEdidInfo->pstVsdbInfo.u8PaAdr, pstEdidInfo->pstVsdbInfo.u8VsdbPaL, pstEdidInfo->pstVsdbInfo.u8VsdbPaH));
	iTE_Edid_Msg(("VSDB Byte6 = 0x%02X, Byte7 = 0x%02X\n\r", pstEdidInfo->pstVsdbInfo.u8B6, pstEdidInfo->pstVsdbInfo.u8B7));
	iTE_Edid_Msg(("ExtVic = 0x%02X, SbS=0x%04X, TaB=0x%04X, FP=0x%04X\n\r", pstEdidInfo->pstVsdbInfo.u8ExtVic, pstEdidInfo->pstVsdbInfo.u163dSbS, pstEdidInfo->pstVsdbInfo.u163dTaB, pstEdidInfo->pstVsdbInfo.u163dFP));
	iTE_Edid_Msg(("Blk0DtdVic = 0x%02X, 0x%02X, 0x%02X, 0x%02X\n\rBlk0UnSupDtd = 0x%02X\n\r", pstEdidInfo->u8Blk0Dtd[0], pstEdidInfo->u8Blk0Dtd[1], pstEdidInfo->u8Blk0Dtd[2], pstEdidInfo->u8Blk0Dtd[3], pstEdidInfo->u8UnSupDtd));
	iTE_Edid_Msg(("Blk1DtdVic = 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X\n\r", pstEdidInfo->u8Blk1Dtd[0], pstEdidInfo->u8Blk1Dtd[1], pstEdidInfo->u8Blk1Dtd[2], pstEdidInfo->u8Blk1Dtd[3], pstEdidInfo->u8Blk1Dtd[4], pstEdidInfo->u8Blk1Dtd[5]));
	iTE_Edid_Msg(("VDB Info:\n\r"));
#if (SAVE_VIC_LIST == iTE_TRUE)
	iTE_Edid_Msg(("NatVic =    0x%02X, 0x%02X, 0x%02X, 0x%02X\n\r", pstEdidInfo->pstVdbInfo.u8NatList[0], pstEdidInfo->pstVdbInfo.u8NatList[1], pstEdidInfo->pstVdbInfo.u8NatList[2], pstEdidInfo->pstVdbInfo.u8NatList[3]));
	iTE_Edid_Msg(("VicList =   %02X %02X %02X %02X %02X %02X %02X %02X ", pstEdidInfo->pstVdbInfo.u8VicList[0], pstEdidInfo->pstVdbInfo.u8VicList[1], pstEdidInfo->pstVdbInfo.u8VicList[2], pstEdidInfo->pstVdbInfo.u8VicList[3],
		pstEdidInfo->pstVdbInfo.u8VicList[4], pstEdidInfo->pstVdbInfo.u8VicList[5], pstEdidInfo->pstVdbInfo.u8VicList[6], pstEdidInfo->pstVdbInfo.u8VicList[7]));
	iTE_Edid_Msg(("%02X %02X %02X %02X %02X %02X %02X %02X\n\r", pstEdidInfo->pstVdbInfo.u8VicList[8], pstEdidInfo->pstVdbInfo.u8VicList[9], pstEdidInfo->pstVdbInfo.u8VicList[10], pstEdidInfo->pstVdbInfo.u8VicList[11],
		pstEdidInfo->pstVdbInfo.u8VicList[12], pstEdidInfo->pstVdbInfo.u8VicList[13], pstEdidInfo->pstVdbInfo.u8VicList[14], pstEdidInfo->pstVdbInfo.u8VicList[15]));
	iTE_Edid_Msg(("3DVicList = %02X %02X %02X %02X %02X %02X %02X %02X ", pstEdidInfo->pstVdbInfo.u8Vic3dList[0], pstEdidInfo->pstVdbInfo.u8Vic3dList[1], pstEdidInfo->pstVdbInfo.u8Vic3dList[2], pstEdidInfo->pstVdbInfo.u8Vic3dList[3],
	...
	iTE_Edid_Msg(("%02X %02X %02X %02X %02X %02X %02X %02X\n\r", pstEdidInfo->pstVdbInfo.u8Vic3dList[8], pstEdidInfo->pstVdbInfo.u8Vic3dList[9], pstEdidInfo->pstVdbInfo.u8Vic3dList[10], pstEdidInfo->pstVdbInfo.u8Vic3dList[11],
...
#endif
	iTE_Edid_Msg(("Vic Cnt = 0x%02X\n\r", pstEdidInfo->pstVdbInfo.u8VicCnt));
//	iTE_Edid_Msg(("Vic Cnt = 0x%02X, Vic Start = 0x%02X\n\r", pstEdidInfo->pstVdbInfo.u8VicCnt, pstEdidInfo->pstVdbInfo.u8VicStart));

#if (PARSE_CEA_ADB == iTE_TRUE)
{
	iTE_u8	u8Temp;
	iTE_Edid_Msg(("SPK Info: 0x%02X\n\r", pstEdidInfo->u8SpkAlloc));

	iTE_Edid_Msg(("ADB Info:\n\r"));
	for(u8Temp = 0; u8Temp < pstEdidInfo->pstAdbInfo.u8AdoCnt; u8Temp++){
		iTE_Edid_Msg(("0x%02X, 0x%02X, 0x%02X\n\r", pstEdidInfo->pstAdbInfo.u8AdoDes[u8Temp][0], pstEdidInfo->pstAdbInfo.u8AdoDes[u8Temp][1], pstEdidInfo->pstAdbInfo.u8AdoDes[u8Temp][2]));
	}
}
#endif
...
//****************************************************************************
void Edid_DtdParse(iTE_u8 u8RdStart, EdidInfo *pstEdidInfo,  iTE_u16 u16EdidBlk, iTE_pu8 pu8EdidBuf)
{
	...
	while(u8RdStart <= u8RdEnd)
	{
		if(u16EdidBlk & 0xFF00)
		{
			...
			{
				iTE_Edid_Msg(("****ERROR****: Edid_DtdParse, EdidBuf = NULL, %X\n\r", u16EdidBlk));
				return;
			}
		}
		else
...
void Edid_CeaPrase(EdidInfo *pstEdidInfo, EdidParStr *pstEdidParPtr, iTE_pu8 u8EdidBuf, iTE_u1 bPDefault3DVic)
{
	...
	switch(pstEdidParPtr->eTag & 0xE0)
	{
		...
		case CEA_TAG_ADB:
			...
			{
				iTE_Edid_Msg(("************************************* ERROR: Wrong ADB Length = %d\n\r", pstEdidParPtr->s8Length));
				pstEdidParPtr->eTag = CEA_TAG_NONE;
...
void Edid_GetDefault3DVic(EdidInfo *pstEdidInfo, iTE_u8 u8Vic, iTE_u8 u8VicIndex)
{
	switch(u8Vic){
		case 0x04:
			if(pstEdidInfo->u8Default3DVic[0] == 0xFF)
			{		
				pstEdidInfo->u8Default3DVic[0] = u8VicIndex;			
			}
			break;
		case 0x05:
			if(pstEdidInfo->u8Default3DVic[1] == 0xFF)
			{		
				pstEdidInfo->u8Default3DVic[1] = u8VicIndex;			
			}
			break;
		case 0x13:
			if(pstEdidInfo->u8Default3DVic[2] == 0xFF)
			{		
				pstEdidInfo->u8Default3DVic[2] = u8VicIndex;			
			}
			break;
		case 0x14:
			if(pstEdidInfo->u8Default3DVic[3] == 0xFF)
			{		
				pstEdidInfo->u8Default3DVic[3] = u8VicIndex;			
			}
			break;
		case 0x20:
			if(pstEdidInfo->u8Default3DVic[4] == 0xFF)
			{		
				pstEdidInfo->u8Default3DVic[4] = u8VicIndex;			
			}
			break;
		default:
			break;
	...
	*pu8Len = Edid_GetBitCnt(u16Temp);
	switch(*pu8Type)
	{
		...
		default:
			...
			if(*pu16SbS &= ~u16Temp)
			{	
				*pu8Len += Edid_GetBitCnt(*pu16SbS) * 2;	
			}
			if(*pu16TaB &= ~u16Temp)
			{	
				*pu8Len += Edid_GetBitCnt(*pu16TaB);	
			}
			if(*pu16FP &= ~u16Temp)
			{	
				*pu8Len += Edid_GetBitCnt(*pu16FP);		
			}
			break;
	}
	return u16Temp;
...
// VDB
	...
	if(pstEdidInfo->pstVsdbInfo.u8B6 || pstEdidInfo->pstVsdbInfo.u8B7)
	{
		...
		if(pstEdidInfo->pstVsdbInfo.u8B7){
			...
			if(pstEdidInfo->pstVsdbInfo.u8ExtVic || (pstEdidInfo->pstVsdbInfo.u83dMulti & 0x80))
			{//pstEdidInfo->pstVsdbInfo.u163dSbS ||  pstEdidInfo->pstVsdbInfo.u163dTaB ||  pstEdidInfo->pstVsdbInfo.u163dFP){
				...
				if(u83dLen > 0x1F){	iTE_Edid_Msg(("ERROR: 3D Length Bigger Than 31\n\r"));	}
